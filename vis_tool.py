...
def main():
    parser = argparse.ArgumentParser(description="Sound Visualization and Analysis Tool")
    parser.add_argument('-i', type=str, required=True, help='Input file (WAV or Python song description)')
    parser.add_argument('-o', type=str, help='Output filename for plots (e.g., waveform.png)')
    parser.add_argument('--plot', type=str, choices=['waveform', 'spectrogram', 'spectrum', 'all'], help='Type of plot to generate')
    parser.add_argument('--analyze', action='store_true', help='Perform analysis and print metrics')
    parser.add_argument('-s', type=int, default=44100, help='Sample rate (for generation)')
    ...
    if args.analyze:
        ...
        print(f"  - ADSR Estimates:      A={adsr['attack']:.3f}s, D={adsr['decay']:.3f}s, S={adsr['sustain_level']:.2f}, R={adsr['release']:.3f}s")
        ...
        print("\n🔹 3. SPECTRAL SHAPE (TIMBRE)")
        print(f"  - Spectral Centroid:   {analyzer.get_spectral_centroid():.2f} Hz (Brightness)")
        print(f"  - Spectral Bandwidth:  {analyzer.get_spectral_bandwidth():.2f} Hz")
        print(f"  - Spectral Rolloff:    {analyzer.get_spectral_rolloff():.2f} Hz (85% Energy)")
        print(f"  - Spectral Flatness:   {analyzer.get_spectral_flatness():.4f} (0=Tone, 1=Noise)")
        ...
        print(f"  - Pitch Clarity:       {analyzer.get_pitch_clarity():.4f} (AutoCorr Peak)")
        print(f"  - HNR:                 {analyzer.get_harmonic_to_noise_ratio():.2f} dB")
        print(f"  - Odd/Even Ratio:      {analyzer.get_odd_even_harmonic_ratio():.4f} (>1 Odd dom, <1 Even dom)")
        t1, t2, t3 = analyzer.get_tristimulus()
        print(f"  - Tristimulus:         T1={t1:.2f} (Fund), T2={t2:.2f} (2-4), T3={t3:.2f} (High)")
        ...
        print(f"  - Clipping (>0.99):    {clipping:.2f}% {'🔴' if clipping > 0 else '✅'}")
        print(f"  - DC Offset:           {dc_offset:.4f} {'🔴' if abs(dc_offset) > 0.01 else '✅'}")
    ...
    if args.plot:
        if args.plot == 'waveform' or args.plot == 'all':
            ...
            if args.plot == 'all': out_name = "waveform.png"
        ...
        if args.plot == 'spectrogram' or args.plot == 'all':
            ...
            if args.plot == 'all': out_name = "spectrogram.png"
        ...
        if args.plot == 'spectrum' or args.plot == 'all':
            ...
            if args.plot == 'all': out_name = "spectrum.png"
