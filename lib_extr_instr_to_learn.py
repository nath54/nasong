...
def TrainableSquareSynth(
    ...
    sustain_level = lv.ValueTrainableParameter(0.7)
    release_time = lv.ValueTrainableParameter(0.2)
    duty_cycle = lv.ValueTrainableParameter(0.5)
    num_harmonics = 15
...
def TrainableKick(
    ...
    env = lv.ExponentialDecay(
        ...
        decay_rate=decay_rate.value.item()
    )
    
    # Frequency sweep envelope (faster decay)
    pitch_env = lv.ExponentialDecay(
        time=time,
        start_time=start_time,
        decay_rate=30.0
    )
    ...
    freq = lv.Sum(
        base_freq,
        lv.Product(freq_sweep_amount, pitch_env)
    )
    ...
    # Mix components
    mixed = lv.Sum(
        lv.Product(tone, lv.Constant(1.0 - noise_amount.value.item())),
        noise,
        click
    )
...
def TrainableHiHat(
    ...
    env = lv.ExponentialDecay(
        time=time,
        start_time=start_time,
        decay_rate=base_decay.value.item()
    )
    ...
    metallic = lv.Sum(
        ...
        lv.Sin(time, lv.Constant(10000.0 * 6.283185307179586), lv.Product(brightness, lv.Constant(0.7))),
        lv.Sin(time, lv.Constant(12000.0 * 6.283185307179586), lv.Product(brightness, lv.Constant(0.5)))
    )
    
    mixed = lv.Sum(
        lv.Product(noise, lv.Constant(0.7)),
        lv.Product(metallic, lv.Constant(0.3))
    )
...
def TrainablePlucked(
    ...
    brightness = lv.ValueTrainableParameter(0.7)  # Controls harmonic falloff
    ...
    pluck_env = lv.ExponentialDecay(
        time=time,
        start_time=start_time,
        decay_rate=pluck_decay.value.item()
    )
...
def TrainablePiano(
    ...
    harmonic2 = lv.Sin(time, lv.Product(freq_rads, lv.Constant(2.0)), lv.Product(brightness, lv.Constant(0.5)))
    harmonic3 = lv.Sin(time, lv.Product(freq_rads, lv.Constant(3.0)), lv.Product(brightness, lv.Constant(0.3)))
...
def TrainableBowed(
    ...
    vibrato = lv.Sin(
        time,
        lv.Product(vibrato_rate, lv.Constant(6.283185307179586)),
        vibrato_depth
    )
    modulated_freq = lv.Product(
        frequency,
        lv.Sum(lv.Constant(1.0), vibrato)
    )
...
def TrainablePad(
    ...
    brightness = lv.ValueTrainableParameter(0.6)
    ...
    osc3 = lv.Sin(
        time,
        lv.Product(freq_rads, lv.Sum(lv.Constant(1.0), lv.Product(detune_amount, lv.Constant(-1.0)))),
        lv.Constant(0.33)
    )
...
def TrainableBass(
    ...
    distorted = lv.Distortion(
        value=mixed,
        gain=distortion_amount
    )
...
def get_trainable_instrument(instrument_name: str):
    ...
    if instrument_name not in TRAINABLE_INSTRUMENTS:
        raise ValueError(f"Unknown instrument: {instrument_name}. Available: {list(TRAINABLE_INSTRUMENTS.keys())}")
