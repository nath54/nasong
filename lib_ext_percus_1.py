...
def _vectorized_noise(
    ...
    #
    return noise_float * 200.0 * scale
...
class Snare(lv.Value):
    ...
    def __getitem__(self, index: int) -> float:
        """Gets the value at a single sample index (slow)."""
        #
        t: float = self.time.__getitem__(index=index)
        relative_time: float = t - self.trigger_time
    ...
    def getitem_np(self, indexes_buffer: NDArray[np.float32]) -> NDArray[np.float32]:
        """Gets the values for an array of sample indexes (vectorized)."""
        #
        t: NDArray[np.float32] = self.time.getitem_np(indexes_buffer=indexes_buffer)
        relative_time: NDArray[np.float32] = t - self.trigger_time
...
class HiHat(lv.Value):
    ...
    def getitem_np(self, indexes_buffer: NDArray[np.float32]) -> NDArray[np.float32]:
        ...
        #
        return amp_env * noise * 0.15 * mask
...
class CrashCymbal(lv.Value):
    ...
    def getitem_np(self, indexes_buffer: NDArray[np.float32]) -> NDArray[np.float32]:
        ...
        #
        return amp_env * noise * 0.25 * mask
