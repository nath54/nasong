...
class AudioAnalyzer:
    ...
    def get_spectral_bandwidth(self) -> float:
        ...
        return float(np.sqrt(np.sum(((freqs - centroid) ** 2) * magnitudes) / np.sum(magnitudes)))
    ...
    def get_adsr_estimates(self) -> dict[str, float]:
        ...
        if start_sustain >= end_sustain:
            ...
            decay_indices = np.where(env_norm[peak_idx:start_sustain] <= sustain_level * 1.1)[0]
    ...
    def get_temporal_kurtosis(self) -> float:
        ...
        if moment2 == 0: return 0.0
        return float(moment4 / (moment2 ** 2)) - 3.0 # Excess kurtosis
    ...
    def get_spectral_flux(self) -> float:
        ...
        frame_size = 1024
        hop_size = 512
        if len(self.audio_data) < frame_size: return 0.0
    ...
    def get_harmonic_to_noise_ratio(self) -> float:
        ...
        if lag >= len(self.audio_data): return 0.0
        ...
        # HNR = R(tau) / (R(0) - R(tau))
        if r0 - r_tau <= 1e-10: return 100.0 # Infinite HNR
    ...
    def get_tristimulus(self) -> tuple[float, float, float]:
        ...
        if total_harmonic_energy == 0: return 0.0, 0.0, 0.0
