...
def song(time: lv.Value) -> lv.Value:
    ...
    bass_notes = [
        ...
        DeepBass(
            time, freq_factor_bass * 55.00, 11.0, duration_factor_bass * 0.5
        ),  # A1
        DeepBass(
            time, freq_factor_bass * 61.74, 11.5, duration_factor_bass * 0.4
        ),  # B1
        # Bar 7
    ...
    drums = lv.Sum(*kicks, *snares, *hihats, *crashes)

    bass = lv.Sum(*bass_notes)

    # Mix everything together
    final = lv.Sum(
        lv.Product(drums, lv.Constant(0.85)), lv.Product(bass, lv.Constant(1.0))
    )
