...
def detect_pitches_fft(
    ...
    for i in range(1, len(mags_filtered) - 1):
        if (mags_filtered[i] > threshold and 
            mags_filtered[i] > mags_filtered[i-1] and 
            mags_filtered[i] > mags_filtered[i+1]):
...
def spectral_loss(
    ...
    synth_stft = torch.stft(synthesized, n_fft=n_fft, hop_length=hop_length, 
    ...
    freq_weights = torch.linspace(1.0, high_freq_emphasis, freq_bins, device=synthesized.device)
...
def multi_resolution_spectral_loss(
    ...
    for n_fft in fft_sizes:
        hop_length = n_fft // 4
        loss = spectral_loss(synthesized, target, sample_rate, n_fft, hop_length, high_freq_emphasis)
...
def collect_trainable_parameters(value: lv.Value, params: List[Tensor] = None) -> List[Tensor]:
    """
...
def train_instrument(
    ...
    # Load target audio
    print(f"Loading audio segment ({segment_start}s - {segment_start + segment_duration}s)...")
    target_audio, sr = load_wav_segment(wav_path, segment_start, segment_duration, sample_rate)
    target_tensor = torch.from_numpy(target_audio).to(dtype=torch.float32, device=device)
    
    # Extract note parameters (frequency, timing)
    print(f"Extracting {num_notes} note(s) from audio...")
    note_params = extract_note_parameters(target_audio, sample_rate, num_notes, use_onset_detection=True)
    
    print("Detected notes:")
    for i, note in enumerate(note_params):
        freq_str = ", ".join([f"{f:.1f}Hz" for f in note['frequencies']])
        print(f"  Note {i+1}: [{freq_str}], start={note['start_time']:.2f}s, dur={note['duration']:.2f}s")
    ...
    for tn in trainable_notes:
        ...
        if instrument_name in ["kick", "snare", "hihat_closed", "hihat_open"]:
            ...
            if len(chord_voices) == 1:
                ...
                note_values.append(lv.Product(lv.Sum(chord_voices), lv.Constant(1.0 / len(chord_voices))))
    ...
    for epoch in range(num_epochs):
        ...
        idx_buffer = torch.arange(len(target_tensor), dtype=torch.float32, device=device)
    ...
    final_audio = synth_output.getitem_torch(final_idx, sample_rate, device=device).detach().cpu().numpy()
...
def main():
    parser = argparse.ArgumentParser(description="Train synthesized instruments from WAV files")
    
    parser.add_argument("wav_file", type=str, help="Path to input WAV file")
    parser.add_argument("--instrument", "-i", type=str, default="sine",
                        choices=list(learnable.TRAINABLE_INSTRUMENTS.keys()),
                        help="Instrument type to train")
    parser.add_argument("--num-notes", "-n", type=int, default=1,
                        help="Number of notes in the segment")
    parser.add_argument("--start", "-s", type=float, default=0.0,
                        help="Start time of segment (seconds)")
